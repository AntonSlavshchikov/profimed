...
export const ReviewForm = () => {
    ...
    const newReview = () => {
        createReview(titleReview, textReview)
    }

    return (
        <div className="write__review">
            <MyButton onClick={openForm} id="reviewBtn">Написать отзыв</MyButton>
            <form className="review__form" id="reviewForm" onSubmit={newReview}>
                <MyInput placeholder="ФИО" value={titleReview} onChange={(e) => setTitleReview(e.target.value)} required />
                <MyInput placeholder="Отзыв" value={textReview}  onChange={(e) => setTextReview(e.target.value)} required />
